...
class afruit_tsl2591_extended():
    ...
    def get_light_levels(self):
        ...
        if self._integration_time == self.INTEGRATIONTIME_100MS:
            max_counts = self._TSL2591_MAX_COUNT_100MS
        else:
            max_counts = self._TSL2591_MAX_COUNT
        ...
        again = 1.0
        if self._gain == self.GAIN_MED:
            again = 25.0
        elif self._gain == self.GAIN_HIGH:
            again = 428.0
        elif self._gain == self.GAIN_MAX:
            again = 9876.0
        cpl = (atime * again) / self._TSL2591_LUX_DF
        lux1 = (channel_0 - (self._TSL2591_LUX_COEFB * channel_1)) / cpl
        lux2 = (
            (self._TSL2591_LUX_COEFC * channel_0) -
            (self._TSL2591_LUX_COEFD * channel_1)
        ) / cpl
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # ir, visible = sensor.raw_luminosity()

            time_stamp = datetime.datetime.now()
            if DEBUG:
                print(time_stamp.strftime("%d/%m/%Y %H:%M:%S.%f")
                      [:-3] + " " + "Total light: {0} lux".format(lux))
